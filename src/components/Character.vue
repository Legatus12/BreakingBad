<template>
    <div class="w-full md:w-fit h-60 md:h-fit overflow-hidden rounded-3xl bg-[#232323] text-[#f6f6f6] text-sm md:text-xl flex flex-row md:flex-col justify-between shrink-0">
        ...
        <div class="w-64 flex flex-col justify-center p-4 gap-4">
            ...
            class="w-full bg-[#2d572c] hover:bg-[#468845] duration-300 text-[#f6f6f6] text-base font-bold rounded-2xl p-2">
                {{ this.info }}
            </button>
            <button @click="clickFav"
            class="w-full bg-[#ffba00] hover:bg-[#ffe190] duration-300 text-[#232323] text-base font-bold rounded-2xl p-2">
                {{ this.isFav }}
            </button>
        </div>
    </div>
...
<script lang="ts">
    export default {
        ...
        methods:{
            clickMore(){
                if(!this.more){this.more = true; this.info = "- info"}
                else{this.more = false; this.info = "+ info"}
            },
            clickFav(){
